...
{ /* Bold Text */ }
<p tw="font-mono text-r font-bold">Hello</p>

{ /* With CSS Props */ }
<p css={[tw\`font-mono text-r\`]}>Hello</p>
`,
    },
    page: () => (
      <Fragment>
        <Title />
        <Subtitle>Mono - monospaceのタイポグラフィ</Subtitle>
        <Description
          markdown={`
3design-uiではサンセリフのフォントに加え，等幅フォントも使うことができます。
...
1. ~~ \`yakuhanjp\` 約物といわれる「や、を詰めるフォントです。~~ \`yakuhanjp\` は等幅フォントでは約物を詰めないため使用しません。
3. \`Fira Mono\` 欧文用のフォントです。\`Fira Code\` ではなく，\`<=\`などをligatureにしない，より一般的に馴染みのあるフォントを選択しています。
2. \`Noto Sans JP\` 和文用のフォントです。
...
- \`700\` 太字
        ...
        />
        <Source />
        <DocsStory id="example-typography--mono" />
      </Fragment>
    ),
  },
}
